...
import '../../features/overview/data/triage_repository.dart' hide TriageCategory;
...
class _AppShellState extends ConsumerState<AppShell> {
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: selected ? Colors.white.withValues(alpha: 0.96) : Colors.transparent,
              borderRadius: BorderRadius.circular(12),
              border: selected ? Border.all(color: Colors.white.withValues(alpha: 0.7)) : null,
              boxShadow: selected
                  ? [
                      BoxShadow(
            ...
            child: Row(
              children: [
                AnimatedContainer(
                  ...
                  height: 18,
                  decoration: BoxDecoration(
                    color: selected ? ClinicalColors.primaryEmerald : Colors.transparent,
                ...
                Icon(
                  item.icon,
                  color: selected ? ClinicalColors.sidebarGradientStart : Colors.white.withValues(alpha: 0.62),
                  size: 20,
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    item.label,
                    style: TextStyle(
                      color: selected ? ClinicalColors.sidebarGradientStart : Colors.white.withValues(alpha: 0.72),
                      fontSize: 13,
  ...
  Widget _buildHeader(bool isMobile) {
    ...
    final patients = ref.watch(triageOverviewProvider).asData?.value ?? const <PatientTriageModel>[];
        ...
        children: [
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: TextStyle(color: ClinicalColors.textMuted.withValues(alpha: 0.8)),
                    prefixIcon: const Icon(Icons.search_rounded, color: ClinicalColors.textMuted, size: 20),
          ...
          Container(
            ...
            decoration: const BoxDecoration(
              shape: BoxShape.circle,
              gradient: LinearGradient(
                colors: [ClinicalColors.primaryEmerald, ClinicalColors.sidebarGradientStart],
              ),
            ),
            child: CircleAvatar(
              radius: 16,
              ...
              child: Text(
                activeRole == StaffRole.physician ? 'MD' : (activeRole == StaffRole.director ? 'DIR' : 'NS'),
                style: const TextStyle(
...
class UrgentTriageView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: triageAsync.when(
        ...
        data: (allPatients) {
          final criticalCases = allPatients.where((p) => _getCategory(p) == TriageCategory.critical).toList();
          final overTreatmentCases = allPatients.where((p) => _getCategory(p) == TriageCategory.overTreatment).toList();
              ...
              children: [
                ...
                if (urgentList.isEmpty)
                  Container(
                    ...
                    child: const Column(
                      children: [
                        ...
                        SizedBox(height: 4),
                        Text('ผู้ป่วยในทะเบียนทุกคนอยู่ในเกณฑ์ควบคุมได้หรือได้รับการดูแลเรียบร้อยแล้ว', style: TextStyle(color: ClinicalColors.textMuted, fontSize: 12)),
                      ],
                    ),
                  )
                else
                  ...urgentList.map((patient) {
                    ...
                    final cardColor = isCritical ? ClinicalColors.criticalRed : const Color(0xFF6366F1);
                    final initials = patient.fullName.trim().isEmpty ? '?' : patient.fullName.trim().characters.first.toUpperCase();
                      ...
                      child: Row(
                        children: [
                          CircleAvatar(
                            ...
                            child: Text(initials, style: TextStyle(fontWeight: FontWeight.bold, color: cardColor)),
                          ),
                          const SizedBox(width: 14),
                          Expanded(
                            child: Column(
                                ...
                                Row(
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: isCritical ? ClinicalColors.criticalBg : const Color(0xFFEEF2FF),
                                        borderRadius: BorderRadius.circular(6),
                                      ),
                                      child: Text(
                                        isCritical ? '🚨 วิกฤต BP สูง' : '⚠️ ความดันตก (Over-Tx)',
                            ...
                            children: [
                              FilledButton.tonal(
                                onPressed: () {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('รับทราบเคสและเริ่มกระบวนการติดตามเรียบร้อยแล้ว'),
                                ...
                                child: const Text('รับเคส (Ack)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                              ),
                              OutlinedButton.icon(
                                onPressed: () => Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (_) => PatientDetailScreen(patientId: patient.patientId),
                                  ),
                                ),
                              ...
                              FilledButton.icon(
                                onPressed: patient.phone == null || patient.phone!.trim().isEmpty
                                    ? null
                                    : () => _callPatient(context, patient.phone!),
                                style: FilledButton.styleFrom(backgroundColor: ClinicalColors.criticalRed),
  ...
  Widget _buildMetricBadge(String label, String value, Color color, Color bg, IconData icon) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: bg,
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: color.withValues(alpha: 0.9))),
            ],
          ),
        ],
      ),
...
class PatientRegistryView extends ConsumerWidget {
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    OutlinedButton.icon(
                      ...
                      onPressed: displayPatients.isEmpty ? null : () => ExportUtil.exportPatientListToCsv(displayPatients),
                      icon: const Icon(Icons.download_rounded, size: 16),
                      label: const Text('ส่งออก CSV', style: TextStyle(fontSize: 12)),
                    ),
                  ],
                ),
                ...
                Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _buildFilterChip(ref, 'ทั้งหมด (${allPatients.length})', 'ALL', selectedFilter),
                    _buildFilterChip(ref, 'วิกฤต', 'CRITICAL', selectedFilter, activeColor: ClinicalColors.criticalRed),
                    _buildFilterChip(ref, 'ความดันตก (Over-Tx)', 'OVER_TREATMENT', selectedFilter, activeColor: const Color(0xFF6366F1)),
                    _buildFilterChip(ref, 'ความดันต่ำทั่วไป', 'HYPOTENSION', selectedFilter, activeColor: const Color(0xFF0284C7)),
                    _buildFilterChip(ref, 'เฝ้าระวัง', 'WARNING', selectedFilter, activeColor: ClinicalColors.warningOrange),
                    _buildFilterChip(ref, 'ปกติ', 'NORMAL', selectedFilter, activeColor: ClinicalColors.normalGreen),
                  ],
                ),
                const SizedBox(height: 16),
                ...
                Container(
                  ...
                  child: displayPatients.isEmpty
                      ? const Padding(
                          ...
                          child: Center(
                            child: Text('ไม่พบข้อมูลผู้ป่วยที่ตรงกับเงื่อนไขการค้นหา', style: TextStyle(color: ClinicalColors.textMuted)),
                          ),
                        )
                      : SingleChildScrollView(
                          ...
                          child: ConstrainedBox(
                            ...
                            child: DataTable(
                              ...
                              columns: const [
                                DataColumn(label: Text('HN', style: TextStyle(fontWeight: FontWeight.bold))),
                                DataColumn(label: Text('ชื่อ - นามสกุล', style: TextStyle(fontWeight: FontWeight.bold))),
                                DataColumn(label: Text('ความดันล่าสุด', style: TextStyle(fontWeight: FontWeight.bold))),
                                DataColumn(label: Text('ชีพจร', style: TextStyle(fontWeight: FontWeight.bold))),
                                DataColumn(label: Text('น้ำตาล FBS', style: TextStyle(fontWeight: FontWeight.bold))),
                                DataColumn(label: Text('สถานะคัดกรอง', style: TextStyle(fontWeight: FontWeight.bold))),
                              ],
                              rows: displayPatients.map((p) {
                                ...
                                } else if (category == TriageCategory.hypotension) {
                                ...
                                return DataRow(
                                  onSelectChanged: (_) {
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
                                        builder: (_) => PatientDetailScreen(patientId: p.patientId),
                                      ),
                                    );
                                  },
                                  cells: [
                                    ...
                                    DataCell(Text(p.pulse != null ? '${p.pulse} bpm' : '-')),
  ...
  Widget _buildFilterChip(WidgetRef ref, String label, String value, String currentSelected, {Color? activeColor}) {
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              ...
              style: TextStyle(color: ClinicalColors.textMuted, fontSize: 13, height: 1.4),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 24),
            FilledButton.icon(
